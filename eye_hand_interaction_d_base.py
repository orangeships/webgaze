...
class EyeHandInteractionSystem:
    ...
    def _check_sliding_window_distribution(self):
        ...
        if max_distance < pixel_threshold and avg_distance < avg_pixel_threshold:
            # 第一个条件已满足：视线稳定
            # 现在检查第二个条件：距离阈值条件
            self._check_distance_threshold_and_move_cursor(center_x, center_y)
    
    def _check_distance_threshold_and_move_cursor(self, target_x, target_y):
        """检查距离阈值条件并移动光标（第二个触发条件）"""
        try:
            # 获取当前鼠标位置
