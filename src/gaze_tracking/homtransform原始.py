...
class HomTransform:
    ...
    def RunGazeOnScreen(self, model, cap, sfm=False):
        ...
        while cap.isOpened():
            ...
            # 在终端输出帧率信息
            if elapsed_time > 1.0:  # 每秒更新一次帧率
                print(f"FPS: {fps:.1f}, Frame: {frame_count}, Status: Tracking")
    ...
    def calibrate(self, model, cap, sfm=False):
        ...
        # 添加半透明背景
        overlay = start_frame.copy()
        cv2.rectangle(overlay, (self.width//4, self.height//3), (3*self.width//4, 2*self.height//3), (255, 255, 255), -1)
        cv2.addWeighted(overlay, 0.7, start_frame, 0.3, 0, start_frame)
        
        # 使用PIL显示中文文本
        start_frame = gcv.put_chinese_text(start_frame, "眼动追踪校准系统", (self.width//3, self.height//3 + 50), font_size=40, color=(0, 0, 0))
        start_frame = gcv.put_chinese_text(start_frame, "按 'S' 键开始校准", (self.width//3, self.height//3 + 100), font_size=30, color=(0, 0, 0))
        start_frame = gcv.put_chinese_text(start_frame, "按 'ESC' 键退出", (self.width//3, self.height//3 + 150), font_size=30, color=(0, 0, 0))
        
        # 绘制开始按钮
        cv2.rectangle(start_frame, (self.width//3, self.height//2), (2*self.width//3, self.height//2 + 60), (0, 200, 0), -1)
        start_frame = gcv.put_chinese_text(start_frame, "开始校准", (self.width//2 - 60, self.height//2 + 40), font_size=30, color=(255, 255, 255))
        
        gcv.display_window(start_frame, "等待开始校准...", False, 0)
        ...
        while True:
            ...
            if key_pressed == ord('s') or key_pressed == ord('S'):
                ...
                print("校准开始...")
                break
            elif key_pressed == 27:  # ESC键
                print("用户取消校准")
                cv2.destroyAllWindows()
                return None
        
        while (cap.isOpened() and calibration_started):
            ...
            # 显示校准进度
            progress_text = f"校准点 {idx+1}/6"
            gcv.display_window(frame2, progress_text, True, valid_frames)
    ...
    def _RemoveOutliers(self):
        ...
        for i in range(idx):            
            ...
            # 对所有三个维度都应用过滤，提高校准精度
            mask = self._MaskOutliers(g_.loc[:,'gaze_x']) & \
                   self._MaskOutliers(g_.loc[:,'gaze_y']) & \
                   self._MaskOutliers(g_.loc[:,'gaze_z'])  # 增加对gaze_z的过滤
            g[i] = g_[mask]
            s[i] = set_val[mask]
            WTG[i] = WTG_[mask]
